                                          ...
                                          T min_diagonal_ratio = 0.25)
{
    ...
    for (unsigned int k = 0; k < matrix.n; ++k) {
        ...
        if (factor.invdiag[k] < min_diagonal_ratio * factor.adiag[k])
            factor.invdiag[k] =
                1 / sqrt(factor.adiag[k]);  // drop to Gauss-Seidel here if the pivot looks dangerously small
        else
...
template <class T>
struct PCGSolver {
    PCGSolver() { set_solver_parameters(1e-10, 150000, 0.97, 0.25); }

    void set_solver_parameters(T tolerance_factor_,
                               int max_iterations_,
    ...
    bool solve(const SparseMatrix<T> &matrix,
